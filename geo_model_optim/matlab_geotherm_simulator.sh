...
cp Geotherm_model.m workdir.$num/

# ------------------------------------
# RUN the simulation from workdir.num
# ------------------------------------
# launch Matlab with command mode (-r)
# Geotherm_model.m will call the actual analysis file
# rosenbrock.m
cd workdir.$num

echo matlab -nojvm -nodesktop -nodisplay -nosplash -r “Geotherm_model(‘params.in', 'results.out'); exit"

matlab -nojvm -nodesktop -nodisplay -nosplash -r “Geotherm_model(‘params.in', 'results.out'); exit"

# Note that you can put other matlab commands in the argument to -r, e.g., 
# problem setup commands:
# -r "x=2; y=sin(x); Geotherm_model(x,y); exit;"
